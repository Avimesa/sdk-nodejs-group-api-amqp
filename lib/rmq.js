 ...
 *
 * @param {String} msg           JSON message to send.
 * @param {Function} cb          cb(err, msg): called on response
 *
...
/**
 * Sends device actuation command
 *
 * Sends a device actuation command
 *
 * @param {String} devId         Device ID.
 * @param {String} cmd           JSON command
 * @param {Function} cb          cb(err, msg): called after sending
 *
 * @return {type} Description.
 */
function sendActuationCmd(devId, cmd, cb) {

	const connParams = config.getConnParams();
	const rmqSettings = config.getRmqSettings();

	const exchangeName = rmqSettings.exchanges.actuation;
	const routingKey = devId;
		...
		if(err){
			...
			conn.createConfirmChannel(function(err, ch) {
				if (err){
					...
					ch.publish(exchangeName, routingKey, new Buffer(JSON.stringify(cmd)), {}, function (err, ok) {
						if(err){
							cb(true, err.message);
						} else {
							cb(false,"");
...
module.exports.sendActuationCmd = sendActuationCmd;
